...
notification.className = fixed top-4 right-4 px-4 py-2 rounded shadow text-white z-50 ${   type === 'success' ? 'bg-green-500' : type === 'error' ? 'bg-red-500' : 'bg-blue-500'   };
...
html2pdf().set({margin:10, filename:'graph_timer.pdf', html2canvas:{scale:2}}).from(canvas).save();
});
}
